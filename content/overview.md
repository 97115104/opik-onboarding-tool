...
**Opik** (by [Comet](https://www.comet.com)) is an open-source platform for building, evaluating, and operating generative AI applications — from RAG chatbots and code assistants to multi-step agentic workflows.

## The problem Opik solves

LLM apps are hard to debug. A single user message may trigger retrieval, tool calls, multiple model requests, and post-processing. Traditional logs show fragments; Opik captures the **full trace** so you can see what happened, measure quality, and improve over time.

## Core capabilities

### Observability & tracing

- Record every LLM call, tool invocation, and agent step in a hierarchical **span tree**
...
- **Datasets & experiments** — batch-evaluate prompts and models against labeled examples
...
- Ingest high-volume production traces (designed for scale)
...
| Option | Best for |
|--------|----------|
| **Self-hosted** (`./opik.sh`) | Local dev, air-gapped, full control — what this onboarding tool uses |
| **Comet Cloud** | Fastest start, managed infrastructure |
| **Enterprise** | Scale, compliance, org-wide rollout |

This wizard runs the **self-hosted** stack next to your machine so you can trace the chat demo and explore the UI before contributing code.

## How this onboarding tool uses Opik

1. **Clone & start** — `deploy-locally.sh` brings up Opik via `opik.sh` in your local Opik checkout
...
4. **Learn & quiz** — overview, knowledge graph, and tour build product context
